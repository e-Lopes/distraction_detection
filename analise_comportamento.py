...
def setup_grafico():
    ...
    ax.set_title('Ângulo da Cabeça (Últimos 60 Frames)')
    ax.set_ylabel('Graus')
    line, = ax.plot([], [], 'b-')
    fig.tight_layout()
    return fig, ax, line
...
def main():
    ...
    if angulos_totais:
        ...
        plt.plot(timestamps, angulos_totais, 'b-', label='Ângulo')
        plt.axhline(ANGULO_ALERTA, color='r', linestyle='--', label='Limite')
        plt.title('Variação Angular da Cabeça')
        plt.ylabel('Graus')
        plt.xlabel('Tempo (s)')
        plt.legend()
