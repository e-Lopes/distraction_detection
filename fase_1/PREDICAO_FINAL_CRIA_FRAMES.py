...
SAVE_FRAMES = True  # Ativar/desativar salvamento de frames
FRAMES_OUTPUT_DIR = "frames_por_estado"  # Pasta base para salvar os frames
...
def setup_output_directories():
    ...
    estados = ["NAO_DETECTADO", "MAOS_FORA", "POSTURA_RUIM", "CELULAR", "NORMAL"]
    
    if not os.path.exists(FRAMES_OUTPUT_DIR):
        os.makedirs(FRAMES_OUTPUT_DIR)
    
    for estado in estados:
        estado_dir = os.path.join(FRAMES_OUTPUT_DIR, estado)
        if not os.path.exists(estado_dir):
            os.makedirs(estado_dir)
    
    return FRAMES_OUTPUT_DIR

def save_frame(frame, estado, frame_counter):
    """Salva o frame no diretório correspondente ao estado"""
    estado_dir = os.path.join(FRAMES_OUTPUT_DIR, estado)
    frame_filename = f"{frame_counter:06d}.jpg"
    frame_path = os.path.join(estado_dir, frame_filename)
...
def process_video(video_path, roi, frame_counter):
    ...
    while cap.isOpened():
        ...
        # Salvar frame se SAVE_FRAMES estiver ativado
        if SAVE_FRAMES:
...
def main():
    ...
    # Configurar diretórios de saída se SAVE_FRAMES estiver ativado
    if SAVE_FRAMES:
    ...
    relatorios = []
    frame_counter = 1  # Contador global de frames
    
    for video_file in video_files:
        print(f"\nProcessando: {video_file}")
        relatorio, frame_counter = process_video(video_file, roi, frame_counter)
        if relatorio:
